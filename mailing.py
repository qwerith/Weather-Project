...
def create_html_table_rows(data):
    ...
    for i in range(length):
        ...
        if count < length:
            table_row = f"""<tr>
                                <td style="padding: 0 20px;">{data[0][count][0].split(" ")[1][:5]}</td>
                                <td style="padding: 0 10% 0 7%;"> {data[0][count][1]["min_temp"]}° </td>
                                <td style="padding: 0 10% 0 7%;"> {data[0][count][1]["max_temp"]}° </td>
                                <td style="padding: 0 10% 0 7%;"> {data[0][count][1]["humidity"]}% </td>
                                <td style="padding: 0 20px;">{data[0][count][1]["conditions"]} </td>
                                <td style="padding: 0 20px;"> {data[0][count][1]["wind_speed"]} m/s</td>
                            </tr>
                        """
...
def send_gmail(data, receiver):
    ...
    html_txt = f"""<!DOCTYPE html>
        ...
        <body style="width:auto;">
            ...
            <table style="padding:10px;color:white;background-color:#212529;border-radius:100px;">
                    <thead style="font-size:130%;">
                        <tr>
                            <th style="padding: 0 20px;">Time </th>
                            <th style="padding: 0 20px;"> Min temp°C </th>
                            <th style="padding: 0 20px;"> Max temp°C </th>
                            <th style="padding: 0 20px;"> Humidity </th>
                            <th style="padding: 0 20px;"> Conditions </th>
                            <th style="padding: 0 20px;"> Wind </th>
                        </tr>
                    </thead>
        ...
        """
    text = MIMEText(plane_txt, 'plain')
    html = MIMEText(html_txt, "html")
    msg.attach(text)
    msg.attach(html)
    #print(msg.as_string())
